<%@page contentType="text/html; charset=UTF-8" language="java"
            ...
            (function (window) {
                ...
                Calculator.prototype.calc = function (key, val) {
                    ...
                    if (key === '=' && !this.stack[0] && this.curr && this.buff[1]) { // repeating '='
                        return (this.buff[1] === 'yx' ? Math.pow(val, this.buff[0]) : this.buff[1] ===
                            'x√y' ?
                            Math.pow(val, 1 / this.buff[0]) : [1] === 'EE' ? val * Math.pow(10,
                                this.buff[0]) :
                    ...
                    if (rank[key] <= rank[this.stack[this.num - 1][1]]) {
                        this.stack[this.num - 1] = [
                            this.stack[this.num - 1][1] === 'yx' ? Math.pow(this.stack[this.num - 1]
                                [0], val) :
                            this.stack[this.num - 1][1] === 'x√y' ? Math.pow(this.stack[this.num -
                                1][0], 1 / val) :
                            this.stack[this.num - 1][1] === 'EE' ? this.stack[this.num - 1][0] *
                            Math.pow(10, val) :
                            eval('(' + this.stack[this.num - 1][0] + ')' + this.stack[this.num - 1]
                                [1] + '(' + val + ')'),
                            key
                        ];
                    }
                ...
                document.addEventListener('keypress', function (e) {
                    ...
                    if ((key.match(/-1$|log2$|2x$/) && !secondActive) || (key.match(
                            /h$|n$|cos$|ex$/) && secondActive)) {
                        keyDown(false, keyBoard['2nd']);
                        doKey('2nd', true);
                    }
                    ...
                    if (key.match(/^[\d|\.]$/) || key === '+/–') {
                        ...
                        switch (key) {
                            ...
                            case 'cosh':
                                ...
                                break;
                            case 'cosh-1':
                                render(2 * Math.log(Math.sqrt((+dispVal + 1) / 2) + Math.sqrt((+dispVal - 1) /
                                    2)) + '');
                                break;
                            case 'tanh':
                                (function (e1, e2) {
    ...
    <div class="pg-tp">
        ...
        <div class="pr-tp-inr">
            <h4>Calculator</h4>
            <span>Some of extra awesome Calculator Style.</span>
        </div>
    </div>
    <!-- Page Top -->
